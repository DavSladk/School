                        ...
                        const float      dt)
{
  ...
  #pragma acc parallel loop present(p_in, p_out) async(TMP)
  for(int i = 0; i < N; i++)
  {
    #pragma acc loop
    for(int j = 0; j < N; i++)
    {
      ...
      // GRAVITY
      float F = -G * p_in.weight[i] * p_in.weight[j] / (r * r + FLT_MIN);
      float nx = F * dx/ (r + FLT_MIN);
      float ny = F * dy/ (r + FLT_MIN);
      float nz = F * dz/ (r + FLT_MIN);

      vx = nx * dt / p_in.weight[i];
      vy = ny * dt / p_in.weight[i];
      vz = nz * dt / p_in.weight[i];
      ...
      // COLLISION
      vx = ((p_in.weight[i] * p_in.vel_x[i] - p_in.weight[j] * p_in.vel_x[i] + 2 * p_in.weight[j] * p_in.vel_x[j]) / (p_in.weight[i] + p_in.weight[j])) - p_in.vel_x[i];
      vy = ((p_in.weight[i] * p_in.vel_y[i] - p_in.weight[j] * p_in.vel_y[i] + 2 * p_in.weight[j] * p_in.vel_y[j]) / (p_in.weight[i] + p_in.weight[j])) - p_in.vel_y[i];
      vz = ((p_in.weight[i] * p_in.vel_z[i] - p_in.weight[j] * p_in.vel_z[i] + 2 * p_in.weight[j] * p_in.vel_z[j]) / (p_in.weight[i] + p_in.weight[j])) - p_in.vel_z[i];
      ...
      p_out.pos_x[i] += vel_x * dt;
      p_out.pos_y[i] += vel_y * dt;
      p_out.pos_z[i] += vel_z * dt;
    }
  }
}
                       ...
                       const int        N)
{
  ...
  #pragma acc parallel loop reduction(+:total_weight, x_center, y_center, z_center) present(p)
  for (int i = 0; i < N; i++)
  {
    total_weight += p.weight[i];
    x_center += p.pos_x[i] * p.weight[i];
    y_center += p.pos_y[i] * p.weight[i];
    z_center += p.pos_z[i] * p.weight[i];
  }

  x_center /= total_weight;
  y_center /= total_weight;
  z_center /= total_weight;

  return {x_center, y_center, z_center, total_weight};
...
void Particles::setCPUMemToZero()
{
  memset(vel_x,  0, size * sizeof(float));
  memset(vel_y,  0, size * sizeof(float));
  memset(vel_z,  0, size * sizeof(float));
  memset(pos_x,  0, size * sizeof(float));
  memset(pos_y,  0, size * sizeof(float));
  memset(pos_z,  0, size * sizeof(float));
  memset(weight, 0, size * sizeof(float));
}
...
Particles::Particles(const Particles &p)
{
  ...
  // logValue = p.logValue;
  allocCPUMemory();
  memcpy(vel_x,  p.vel_x,  size * sizeof(float));
  memcpy(vel_y,  p.vel_y,  size * sizeof(float));
  memcpy(vel_z,  p.vel_z,  size * sizeof(float));
  memcpy(pos_x,  p.pos_x,  size * sizeof(float));
  memcpy(pos_y,  p.pos_y,  size * sizeof(float));
  memcpy(pos_z,  p.pos_z,  size * sizeof(float));
  memcpy(weight, p.weight, size * sizeof(float));
  allocGPUMemory();
}
